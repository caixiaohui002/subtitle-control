    ...
    async start(controller) {
      try {
        ...
        for (let i = 0; i < linesWithoutPunctuation.length; i++) {
          checkAborted();
          const line = linesWithoutPunctuation[i];
          ...
          // 发送进度
          if (i % 5 === 0 || i === linesWithoutPunctuation.length - 1) {
            const message = `正在检测行长度... (${i + 1}/${linesWithoutPunctuation.length})，发现 ${longLines.length} 行超长`;
            controller.enqueue(encoder.encode(`data: ${JSON.stringify({ type: 'progress', step: '步骤4/5', current: i + 1, total: linesWithoutPunctuation.length, message })}\n\n`));
          }
        }
        ...
        if (longLines.length > 0) {
          ...
          for (let i = 0; i < longLines.length; i++) {
            checkAborted();
            const { line } = longLines[i];
            
            try {
              const splitLines = await splitWithLLM(line, maxCharsValue);
              finalLines.push(...splitLines);
              firstRoundCount++;
              sendDebug(`LLM 拆分行 ${i + 1}/${longLines.length}: ${line.substring(0, 20)}... -> ${splitLines.length} 行`);
            } catch (error) {
            ...
            // 发送进度
            if (i % 2 === 0 || i === longLines.length - 1) {
              checkAborted();
              const message = `正在 LLM 语义拆分... (${i + 1}/${longLines.length})`;
              controller.enqueue(encoder.encode(`data: ${JSON.stringify({ type: 'progress', step: '步骤4/5', current: i + 1, total: longLines.length, message })}\n\n`));
            }
          }
        }
        ...
        // 发送结果
        controller.enqueue(encoder.encode(`data: ${JSON.stringify({ type: 'result', content: result })}\n\n`));
